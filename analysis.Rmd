...
useids <- c(ref$from_user, ill$from_user, immi$from_user, gen$from_user)

uniq_tweetids <- unique(tweetids)
uniq_usids <- unique(useids)

length(uniq_tweetids)

newids <- sprintf("j%020d", sample(1:length(uniq_tweetids), replace = F))
newusids <- sprintf("u%010d", sample(1:length(uniq_usids), replace = F))

id_df <- tibble(tweet_id_str = uniq_tweetids, new_ids = newids)
us_df <- tibble(from_user = uniq_usids, newus_ids = newusids)

ref2 <- ref %>% left_join(id_df) %>% left_join(us_df) %>% select(-c(tweet_id_str, from_user)) %>% rename(tweet_id_str = new_ids, from_user = newus_ids)
ill2 <- ill %>% left_join(id_df) %>% left_join(us_df) %>% select(-c(tweet_id_str, from_user)) %>% rename(tweet_id_str = new_ids, from_user = newus_ids)
immi2 <- immi %>% left_join(id_df) %>% left_join(us_df) %>% select(-c(tweet_id_str, from_user)) %>% rename(tweet_id_str = new_ids, from_user = newus_ids)
gen2 <- gen %>% left_join(id_df) %>% left_join(us_df) %>% select(-c(tweet_id_str, from_user)) %>% rename(tweet_id_str = new_ids, from_user = newus_ids)
```
```{r}
...
ref_ang2 <- ref_ang %>% left_join(id_df) %>% left_join(us_df) %>% select(-c(tweet_id_str, from_user)) %>% rename(tweet_id_str = new_ids, from_user = newus_ids)
ill_ang2 <- ill_ang%>% left_join(id_df) %>% left_join(us_df) %>% select(-c(tweet_id_str, from_user)) %>% rename(tweet_id_str = new_ids, from_user = newus_ids)
immi_ang2 <- immi_ang %>% left_join(id_df) %>% left_join(us_df) %>% select(-c(tweet_id_str, from_user)) %>% rename(tweet_id_str = new_ids, from_user = newus_ids)
...
gen_ang2 <- gen_ang %>% left_join(id_df) %>% left_join(us_df) %>% select(-c(tweet_id_str, from_user)) %>% rename(tweet_id_str = new_ids, from_user = newus_ids)
```
