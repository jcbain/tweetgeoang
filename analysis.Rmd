...
# ANALYSIS
We can start out simple...let's see if there is a spatial autocorrelation in the anger data.
```{r}
# set priors, here I'm thinking about diffuse priors that might work for you
priors <- c( set_prior("normal(0,10)", class = "b", coef = "hun_kilo"),
             set_prior("normal(0, 10)", class = "Intercept")
             # can also put a different prior on sigma here if you want
)
tmp_mod <- brm(ind_perc ~ hun_kilo, prior = priors, data = ang_sf, cores = 4)
```


```{r}
ang_sf %>% 
  ggplot(aes(hun_kilo, ind_perc)) + 
  geom_point() +
  stat_smooth(method = "lm") +
  theme_minimal()
ang %>% 
  left_join(cities_sf) %>% 
  ggplot(aes(distance_border, gen_index)) + 
  geom_point() +
  stat_smooth(method = "lm") +
  theme_minimal() 
```

[Difference Between CAR and SAR](https://stats.stackexchange.com/questions/277/spatial-statistics-models-car-vs-sar)
```{r}
...
find_spatial_neighbors <- function(coords, type = "gc"){
  if( type == "gc"){
    gck1 <- knn2nb(knearneigh(coords, k=1, longlat=TRUE))
    all.linked <- max(unlist(nbdists(gck1, coords, longlat=TRUE)))
    nb <- dnearneigh(coords, 0, all.linked, longlat=TRUE)
  } else {
    llk1 <- knn2nb(knearneigh(coords, k=1, longlat=FALSE))
    all.linked <- max(unlist(nbdists(llk1, coords, longlat=FALSE)))
    nb <- dnearneigh(coords, 0, all.linked, longlat=FALSE)
  }
  nb
}
```
